...
class ReportService:
    ...
    def export_to_excel(self, report_data: List[Dict[str, Any]], summary: Dict[str, Any], 
        ...
        try:
            ...
            if report_data:
                ...
                for row in report_data:
                    export_data.append({
                        ...
                        'วันที่/เวลา': row['scan_date'].strftime("%Y-%m-%d %H:%M:%S") if isinstance(row['scan_date'], datetime) else str(row['scan_date']),
                ...
                df = pd.DataFrame(columns=['บาร์โค้ด', 'วันที่/เวลา', 'ประเภทงานหลัก', 'ประเภทงานย่อย', 'หมายเหตุ', 'ผู้ใช้'])
            ...
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                summary_data = {
                    ...
                    'ค่า': [
                        summary['report_date'],
                        summary['job_type_name'],
                        summary['sub_job_type_name'],
                        summary['note_filter'] or 'ไม่มี',
                        summary['total_count'],
            ...
            return {
                'success': True,
                'message': f'ส่งออกรายงานสำเร็จ: {filename}'
            }
